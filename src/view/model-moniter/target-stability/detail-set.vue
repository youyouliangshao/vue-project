			...
			>
				...
				>
					<template scope="scope">
						<div v-if="scope.row.paramType == 'A'">{{scope.row.name}}</div>
						<div v-else class="tr">{{scope.row.name}}</div>
					</template>
				</el-table-column>
				<el-table-column label="建模时点">
					...
					>
						<template scope="scope">
							<el-form v-if="scope.row.paramType == 'B'" @submit.native.prevent>
								<el-form-item>
									<el-input v-validate="rules.moreThanZero" data-vv-value-path="currentValue" data-vv-name="建模时点参数" v-if="scope.row.paramType == 'B'" v-model="scope.row.paramValue"></el-input>
								</el-form-item>
							</el-form>
						</template>
					</el-table-column>
					</el-table-column>
				<!--<el-form-item label="基准分：" :class="{'is-error': errors.has('base-form.基准分')}">
					<el-input data-vv-scope="base-form" v-validate="rules.scoreBase" data-vv-value-path="currentValue" data-vv-name="基准分"
					          v-model="model.logicBaseScore" class="clearwidth"></el-input>
					<i class="c-red">*</i>
					<div v-show="errors.has('base-form.基准分')" class="el-form-item__error">{{ errors.first('base-form.基准分') }}</div>
				</el-form-item>-->
			</el-table>
		</div>
	</div>
